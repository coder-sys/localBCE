...
$circuit = Join-Path $RepoRoot "zk-production-binding\circuits\production_binding_v1.circom"
$build = Join-Path $RepoRoot "zk-production-binding\build-v1"
$inputs = Join-Path $RepoRoot "zk-production-binding\inputs-v1"
$report = Join-Path $RepoRoot "zk-production-binding\production_binding_v1_run_report.json"

New-Item -ItemType Directory -Force -Path $build | Out-Null
Remove-Item -Recurse -Force (Join-Path $build "*") -ErrorAction SilentlyContinue

& $node (Join-Path $RepoRoot "zk-production-binding\scripts\make_production_binding_v1_inputs.js")
& $circom $circuit --r1cs --wasm --sym -o $build

$wasm = Join-Path $build "production_binding_v1_js\production_binding_v1.wasm"
...
$json = [pscustomobject]@{
    circuit = "zk-production-binding/circuits/production_binding_v1.circom"
    status = "compiled-circuit-membership-binding-checks-pending-crypto-audit"
    ...
    caveats = @(
        ...
        "The verifier/contract must pin oracleFactsRoot, feeScheduleRoot, addressBookRoot, rulesetRoot, and nullifierRootBefore to governed on-chain state.",
        "The nullifier transition is an auditable sparse-slot/indexed construction, but still pending external cryptographic audit.",
        "The circuit prevents duplicate approval by making replayed nullifier proofs invalid; it does not separately produce a duplicate-denial proof in this v1 lane."
    )
}
...
if ($failed.Count -gt 0) {
    Write-Error "production binding v1 checks did not match expected pass/fail outcomes"
}
