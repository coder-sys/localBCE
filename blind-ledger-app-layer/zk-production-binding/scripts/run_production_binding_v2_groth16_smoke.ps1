...
$report = Join-Path $RepoRoot "zk-production-binding\production_binding_v2_proof_report.json"

if (-not (Test-Path $r1cs) -or -not (Test-Path $wtns)) {
    & (Join-Path $RepoRoot "zk-production-binding\scripts\run_production_binding_v2_checks.ps1") -RepoRoot $RepoRoot | Out-Null
}

$pot0 = Join-Path $build "pot16_0000.ptau"
$pot1 = Join-Path $build "pot16_0001.ptau"
$potFinal = Join-Path $build "pot16_final.ptau"
$zkey0 = Join-Path $build "production_binding_v2_0000.zkey"
$zkeyFinal = Join-Path $build "production_binding_v2_final.zkey"
...
& $node $snarkjs powersoftau contribute $pot0 $pot1 --name="local production binding v2 test contribution" -e="local deterministic non-production entropy"
& $node $snarkjs powersoftau prepare phase2 $pot1 $potFinal
& $node $snarkjs groth16 setup $r1cs $potFinal $zkey0
& $node $snarkjs zkey contribute $zkey0 $zkeyFinal --name="local v2 zkey contribution" -e="local deterministic non-production zkey entropy"
...
$json = [pscustomobject]@{
    status = "v2-groth16-smoke-proof-generated-and-verified-pending-crypto-audit"
    caveat = "This smoke proof is Groth16 over the Circom production-binding v2 circuit. It is not post-quantum and the local setup is not a production ceremony."
