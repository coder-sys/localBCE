...
/*
  Production binding v1.

  This circuit closes the v0 "prover-chosen root" holes:
  ...
  - nullifier must be derived from claim facts and inserted into an empty sparse slot,
...
template ProductionBindingV1(depth) {
    ...
    nullifierHash.inputs[0] <== rawClaimHash;
    nullifierHash.inputs[1] <== providerKey;
    nullifierHash.inputs[2] <== serviceDate;
    nullifierHash.inputs[3] <== procedureCode;
    ...
    component claimLeafHash = Poseidon(10);
    claimLeafHash.inputs[0] <== rawClaimHash;
    claimLeafHash.inputs[1] <== memberKey;
    claimLeafHash.inputs[2] <== providerKey;
    claimLeafHash.inputs[3] <== memberIdPresent;
    claimLeafHash.inputs[4] <== providerNpiPresent;
    claimLeafHash.inputs[5] <== serviceLinesPresent;
    claimLeafHash.inputs[6] <== diagnosisPresent;
    claimLeafHash.inputs[7] <== totalChargeCents;
    claimLeafHash.inputs[8] <== serviceDate;
    claimLeafHash.inputs[9] <== procedureCode;

    component claimRootHash = Poseidon(1);
    claimRootHash.inputs[0] <== claimLeafHash.out;
    claimRoot === claimRootHash.out;

    component resultHash = Poseidon(8);
    ...
    resultHash.inputs[4] <== feeScheduleRoot;
    resultHash.inputs[5] <== addressBookRoot;
    resultHash.inputs[6] <== rulesetRoot;
    resultHash.inputs[7] <== nullifierHash.out;
    ...
    component combinedHash = Poseidon(13);
    ...
    combinedHash.inputs[7] <== feeScheduleRoot;
    combinedHash.inputs[8] <== addressBookRoot;
    combinedHash.inputs[9] <== rulesetRoot;
    combinedHash.inputs[10] <== verifierKeyId;
    combinedHash.inputs[11] <== decision;
    combinedHash.inputs[12] <== paymentAmountCents;
