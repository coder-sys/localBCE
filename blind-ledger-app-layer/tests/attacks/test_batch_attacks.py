...
class PythonBatchAttackTests(unittest.TestCase):
    def test_a1_double_pay_same_claim_across_two_batches_fail_closed_no_stub_payment(self):
    ...
    def test_real_nullifier_circuit_fresh_duplicate_and_tampered_paths(self):
        ctx = context(sample_payload("BL-NULLIFIER-REAL"))
        nullifier = duplicate_check_nullifier(ctx, "original")
        tree = IndexedNullifierTree()
        runner = NullifierProofRunner()

        fresh = tree.prove_nonmembership(nullifier)
        fresh_result = runner.prove_and_verify(fresh, "attack-A-fresh")
        self.assertTrue(fresh_result.accepted)
        self.assertGreater(fresh_result.proof_size_bytes, 700)
        self.assertLess(fresh_result.proof_size_bytes, 900)

        tree.insert(nullifier)
        with self.assertRaises(DuplicateNullifierError):
            tree.prove_nonmembership(nullifier)
        duplicate_reject = runner.expect_reject(tree.duplicate_rejection_witness(nullifier), "attack-B-duplicate")
        self.assertIn("witness generation failed", duplicate_reject)

        tampered = replace(fresh, pathElements=[fresh.pathElements[0] + 1, *fresh.pathElements[1:]])
        tampered_reject = runner.expect_reject(tampered, "attack-C-tampered")
        self.assertIn("witness generation failed", tampered_reject)
