...
from app.batch.poseidon import FIELD_MODULUS as BN254_FIELD_MODULUS
...
class NewArchitectureSecurityHuntTests(unittest.TestCase):
    def test_cairo_and_bn254_field_mismatch_is_explicit_design_blocker(self):
        self.assertGreater(BN254_FIELD_MODULUS, STARK_FIELD_MODULUS)

        bn254_valid_nullifier = STARK_FIELD_MODULUS + 12345
        stark_native_nullifier = 12345

        self.assertLess(bn254_valid_nullifier, BN254_FIELD_MODULUS)
        self.assertNotEqual(bn254_valid_nullifier, stark_native_nullifier)
        self.assertEqual(bn254_valid_nullifier % STARK_FIELD_MODULUS, stark_native_nullifier)
