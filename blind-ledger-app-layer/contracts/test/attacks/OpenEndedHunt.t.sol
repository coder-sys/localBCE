...
contract OpenEndedHuntTest {
    ...
    function testRegistryRejectsRevokingLastAdmin() public {
        BatchClaimsRegistry registry = new BatchClaimsRegistry(address(this), CHECKER_ID, new OpenEndedAcceptingVerifier(), true);
    ...
    function testPaddingZeroResultLeafPastClaimCountBlocked() public {
        ...
        (,,,,,,,,,, uint256 storedClaimCount,,,) = registry.batches(submission.batchId);
        require(storedClaimCount == 1, "wrong test setup");
    }
    ...
    function testClaimCountAboveMerkleCapacityBlocked() public {
        ...
        (bool exists,,,,,,,,,,,,,) = registry.batches(submission.batchId);
        require(!exists, "over-capacity batch was stored");
    }
