...
from app.batch.poseidon import FIELD_MODULUS, HELPER_CRATE, HELPER_CRATE_VERSION, poseidon_hash
...
def batch_nullifier_commitment(nullifiers: Iterable[int]) -> str:
    ...
    for nullifier in nullifiers:
        acc = poseidon_hash([acc, int(nullifier)])
    return f"{acc:064x}"
