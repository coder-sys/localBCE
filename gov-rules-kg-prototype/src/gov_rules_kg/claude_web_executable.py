...
def write_claude_web_executable_candidates(workdir: Path) -> dict:
    ...
    executable = [normalize_executable_candidate(candidate) for candidate in candidates if isinstance(candidate, dict)]
...
def normalize_executable_candidate(candidate: dict) -> dict:
    ...
    item = {
        ...
        "validation_status": "promotion_ready_not_legal_verified",
        "notes": [
...
def build_executable_summary(input_candidates: list[Any], executable: list[dict]) -> dict:
    ...
    return {
        ...
        "warning": "These are promotion-ready candidates, not legally verified deterministic rules.",
    }
