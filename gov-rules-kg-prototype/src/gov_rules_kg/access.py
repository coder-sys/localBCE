...
def phase0_access_check(config: RunConfig, sources: Iterable[Source] | None = None) -> dict:
    ...
    with httpx.Client(headers=headers, timeout=config.timeout_seconds, follow_redirects=True) as client:
    ...
    report = {
        ...
        "egress_ip": extract_egress_ip(results, selected_sources),
...
def extract_egress_ip(results: list[AccessVerdict], sources: list[Source]) -> str | None:
    for verdict, source in zip(results, sources):
        if source.name == "httpbin_ip" and verdict.reachable:
            try:
                payload = json.loads(verdict.body_excerpt)
                return payload.get("origin")
            except json.JSONDecodeError:
                return verdict.body_excerpt[:100] or None
    return None
