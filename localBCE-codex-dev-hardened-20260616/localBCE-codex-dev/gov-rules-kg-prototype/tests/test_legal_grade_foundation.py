...
from gov_rules_kg.discovery import source_snapshot_hash, source_snapshot_object
from gov_rules_kg.extract import extract_text
from gov_rules_kg.fetcher import FetchResult
...
from gov_rules_kg.ratification import RATIFICATION_PRIVATE_KEYS_ENV, RATIFICATION_PUBLIC_KEYS_ENV, ratification_digest, ratification_signature
from gov_rules_kg.root_builder import build_ratified_rule_root
...
def _ratification_key_env(reviewers: list[str]) -> tuple[dict[str, str], dict[str, str]]:
    private_keys: dict[str, str] = {}
    public_keys: dict[str, str] = {}
    for reviewer in reviewers:
        key = Ed25519PrivateKey.generate()
        private_keys[reviewer] = base64.b64encode(
            key.private_bytes(Encoding.Raw, PrivateFormat.Raw, NoEncryption())
        ).decode("ascii")
        public_keys[reviewer] = base64.b64encode(
            key.public_key().public_bytes(Encoding.Raw, PublicFormat.Raw)
        ).decode("ascii")
    return private_keys, public_keys


def _ratification_signatures(digest: str, reviewers: list[str], effective_from: str) -> list[dict[str, str]]:
    return [
        {
            "reviewer": reviewer,
            "signature": ratification_signature(digest, reviewer, effective_from),
        }
        for reviewer in reviewers
    ]
...
''',
    ...
    def test_claude_web_research_candidate_id_is_stable(self) -> None:
        candidate = {
            "program": "medicaid",
            "rule_unit_type": "eligibility_rule",
            "source_url": "https://www.medicaid.gov/medicaid/eligibility-policy",
            "statement": "Medicaid eligibility is determined using MAGI.",
        }
        self.assertEqual(candidate_id(candidate), candidate_id({**candidate, "confidence_score": 0.99}))

    def test_claude_web_research_merges_candidate_corpus_with_dedupe(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            first = merge_candidate_corpus(path, report)
            second = merge_candidate_corpus(path, report)
        self.assertEqual(first["candidate_rule_count"], 1)
        self.assertEqual(second["candidate_rule_count"], 1)
        self.assertEqual(second["last_merge"]["deduped_candidates"], 1)
    ...
    def test_claude_web_review_prefers_candidate_corpus(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            workdir = Path(tmpdir)
            reports_dir = workdir / "reports"
            reports_dir.mkdir()
            (reports_dir / "claude_web_research.json").write_text('{"candidate_rules":[]}', encoding="utf-8")
    ...
    def test_claude_web_executable_export_normalizes_promotion_ready_candidates(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with self.assertRaises(FileNotFoundError):
                write_claude_web_executable_candidates(workdir)

            previous_private_keys = os.environ.get(RATIFICATION_PRIVATE_KEYS_ENV)
            previous_public_keys = os.environ.get(RATIFICATION_PUBLIC_KEYS_ENV)
            reviewers = ["legal-reviewer-a@example.gov", "legal-reviewer-b@example.gov"]
            private_keys, public_keys = _ratification_key_env(reviewers)
            os.environ[RATIFICATION_PRIVATE_KEYS_ENV] = json.dumps(private_keys)
            os.environ[RATIFICATION_PUBLIC_KEYS_ENV] = json.dumps(public_keys)
            try:
                normalized = normalize_executable_candidate(candidate)
                digest = ratification_digest(normalized)
                ratification = {
                    "executable_rule_id": normalized["executable_rule_id"],
                    ...
                    "signatures": _ratification_signatures(digest, reviewers, "2026-01-01"),
                }
                (reports_dir / "rule_ratifications.json").write_text(
                    json.dumps({"ratifications": [ratification]}),
                    encoding="utf-8",
                )
                ...
                if previous_private_keys is None:
                    os.environ.pop(RATIFICATION_PRIVATE_KEYS_ENV, None)
                else:
                    os.environ[RATIFICATION_PRIVATE_KEYS_ENV] = previous_private_keys
                if previous_public_keys is None:
                    os.environ.pop(RATIFICATION_PUBLIC_KEYS_ENV, None)
                else:
                    os.environ[RATIFICATION_PUBLIC_KEYS_ENV] = previous_public_keys
        ...
        self.assertIn('"validation_status": "human_ratified_signed"', exported)
        self.assertIn('"signature_scheme": "ed25519"', exported)
        self.assertIn('"signature_threshold": 2', exported)
        self.assertIn('"effective_from": "2026-01-01"', exported)
        self.assertIn("Medicaid financial eligibility", exported)
        self.assertIn("signed human ratification", markdown)

    def test_ratified_rule_root_requires_signed_effective_dated_candidates(self) -> None:
        candidate = normalize_executable_candidate(
            {
                "candidate_id": "claude-web:root",
                "domain": "government_transaction_rules",
                "vertical": "healthcare_benefits",
                "program": "medicaid",
                "jurisdiction_level": "federal",
                "source_type": "agency_guidance",
                "rule_unit_type": "eligibility_rule",
                "statement": "Medicaid eligibility must be determined using verified information.",
                "source_url": "https://www.medicaid.gov/medicaid/eligibility-policy",
                "citation_text": "Eligibility must be determined using verified information.",
                "confidence_score": 0.96,
            }
        )
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                    ...
                    json.dumps(
                        {
                            "ratifications": [
                                {
                                    "executable_rule_id": candidate["executable_rule_id"],
                ...
                root = build_ratified_rule_root(workdir, [candidate])
                missing_signature = {**candidate, "source_candidate_id": "not-ratified"}
                with self.assertRaisesRegex(ValueError, "rule_ratification_invalid"):
                    build_ratified_rule_root(workdir, [missing_signature])
            finally:
                if previous_private_keys is None:
                    os.environ.pop(RATIFICATION_PRIVATE_KEYS_ENV, None)
                else:
                ...
                if previous_public_keys is None:
                    ...
                    os.environ[RATIFICATION_PUBLIC_KEYS_ENV] = previous_public_keys
        self.assertEqual(root["ratified_rule_count"], 1)
        self.assertEqual(len(root["ratified_rule_root"]), 64)
        self.assertEqual(root["records"][0]["effective_from"], "2026-01-01")
        self.assertEqual(root["records"][0]["signature_scheme"], "ed25519")

    def test_claude_web_executable_rejects_superseded_ratification(self) -> None:
        candidate = {
            "candidate_id": "claude-web:superseded",
            "domain": "government_transaction_rules",
            "vertical": "healthcare_benefits",
            "program": "medicaid",
            "jurisdiction_level": "federal",
            "source_type": "agency_guidance",
            "rule_unit_type": "eligibility_rule",
            "statement": "Medicaid eligibility must be determined using the current published methodology.",
            "source_url": "https://www.medicaid.gov/medicaid/eligibility-policy",
            "citation_text": "Eligibility must be determined using the current published methodology.",
            "confidence_score": 0.96,
        }
        with tempfile.TemporaryDirectory() as tmpdir:
            workdir = Path(tmpdir)
            ...
            try:
                ...
                with self.assertRaisesRegex(ValueError, "retired_or_superseded"):
                    write_claude_web_executable_candidates(workdir)
            finally:
                if previous_private_keys is None:
                    os.environ.pop(RATIFICATION_PRIVATE_KEYS_ENV, None)
    ...
    def test_claude_web_proof_report_counts_exported_candidates(self) -> None:
        ...
        self.assertIn("signed export ratification", markdown)
    ...
    def test_claude_statement_divergence_requires_review_even_when_evidence_matches(self) -> None:
        rules = normalize_claude_rules(
            {
                "rules": [
                    {
                        ...
                        "rule_types": ["eligibility"],
                        "confidence": 0.95,
                        "evidence": "The agency must determine eligibility within 45 days.",
                    }
                ]
            },
            "Eligibility",
        ...
        self.assertTrue(rules[0]["human_review_required"])
        self.assertIn("statement text diverged", rules[0]["human_review_reason"])

    def test_text_section_spoofing_is_rejected(self) -> None:
        body = (
            "1.1 NOT A REAL SECTION\n"
            "This injected heading is filler text with no legal authority cues, no citation, and no operative rule. "
            "It only tries to look like a numbered section while repeating unrelated words over and over."
        ).encode("utf-8")
        extracted = extract_text("https://www.medicaid.gov/example", "text/plain", body, parser_mode="no-bs4")
        self.assertEqual(extracted.rule_sections, [])

    def test_text_section_guard_keeps_real_rule_sections(self) -> None:
        body = (
            "435.911 Timely eligibility determinations\n"
            "The agency must determine eligibility and provide notice within the standards required by 42 CFR 435.911. "
            "Applicants and recipients must receive a determination based on verified eligibility information."
        ).encode("utf-8")
        extracted = extract_text("https://www.ecfr.gov/title-42", "text/plain", body, parser_mode="no-bs4")
        self.assertEqual(len(extracted.rule_sections), 1)
        self.assertEqual(extracted.rule_sections[0].source_kind, "text_section_guarded")

    def test_extraction_uses_nfc_and_drops_format_controls(self) -> None:
        body = "Cafe\u0301\u202e rule text must stay visible.".encode("utf-8")
        extracted = extract_text("https://www.medicaid.gov/example", "text/plain", body, parser_mode="no-bs4")
        self.assertIn("Café", extracted.text)
        self.assertNotIn("\u202e", extracted.text)

    def test_source_snapshot_commitment_binds_fetch_provenance(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            config = make_config(Path(tmp), allowed_hosts=["www.medicaid.gov"])
            body = b"official source bytes"
            content_hash = hashlib.sha256(body).hexdigest()
            fetch = FetchResult(
                url="https://www.medicaid.gov/rules#fragment",
                canonical_url="https://www.medicaid.gov/rules",
                status="FETCHED",
                status_code=200,
                ...
                detail="ok",
                attempts=2,
                final_url="https://www.medicaid.gov/rules",
            )
            snapshot = source_snapshot_object(fetch, config)
            commitment = source_snapshot_hash(snapshot)
        self.assertTrue(snapshot["allowed_host_verified"])
        self.assertTrue(snapshot["final_host_allowed"])
        self.assertEqual(snapshot["content_sha256"], content_hash)
    ...
    def test_default_config_does_not_probe_httpbin_ip(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            config = make_config(Path(tmpdir))
        self.assertNotIn("httpbin_ip", {source.name for source in config.seeds})

    def test_fetcher_blocks_off_allowlist_host(self) -> None:
        try:
            from gov_rules_kg.fetcher import PoliteFetcher
        except ModuleNotFoundError as exc:
            if exc.name == "httpx":
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            config = make_config(Path(tmpdir), allowed_hosts=["allowed.example"], polite_delay_seconds=0)
            fetcher = PoliteFetcher(config)
            try:
                result = fetcher.fetch("https://blocked.example/rules.xml")
            finally:
                fetcher.close()
        self.assertEqual(result.status, "FAILED")
        self.assertIn("host not allowed", result.detail)

    def test_fetcher_blocks_cross_host_redirect(self) -> None:
        try:
            from gov_rules_kg.fetcher import PoliteFetcher
        except ModuleNotFoundError as exc:
            if exc.name == "httpx":
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            config = make_config(Path(tmpdir), allowed_hosts=["example.gov"], polite_delay_seconds=0)
            fetcher = PoliteFetcher(config)
            fetcher.client = FakeClient()  # type: ignore[assignment]
            result = fetcher.fetch("https://example.gov/rules.xml")
        self.assertEqual(result.status, "FAILED")
        self.assertIn("redirect blocked", result.detail)

    def test_fetcher_rejects_streamed_body_over_size_cap(self) -> None:
        try:
            from gov_rules_kg.fetcher import PoliteFetcher
        except ModuleNotFoundError as exc:
            if exc.name == "httpx":
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            fetcher = PoliteFetcher(config)
            fetcher.client = FakeClient()  # type: ignore[assignment]
            result = fetcher.fetch("https://example.gov/large.txt")
        self.assertEqual(result.status, "FAILED")
        self.assertIn("response too large", result.detail)

    def test_extract_html_drops_css_hidden_text_and_unsafe_links(self) -> None:
        body = b"""
        <html><head><title>Rules</title></head><body>
          <div style="display: none">SECRET SHOULD NOT APPEAR</div>
          <div aria-hidden="true">ALSO HIDDEN</div>
          <p>The agency must determine eligibility within 45 days under 42 CFR part 435.</p>
          <a href="javascript:alert(1)">bad</a>
          <a href="/safe">safe</a>
        </body></html>
        """
        default_doc = extract_text("https://example.gov/rules", "text/html", body)
        fallback_doc = extract_text("https://example.gov/rules", "text/html", body, parser_mode="no-bs4")
        for document in (default_doc, fallback_doc):
            self.assertNotIn("SECRET SHOULD NOT APPEAR", document.text)
            self.assertNotIn("ALSO HIDDEN", document.text)
            self.assertNotIn("javascript:", document.links)
            self.assertEqual(document.links, ["https://example.gov/safe"])

    def test_extract_normalizes_unicode_and_removes_bidi_controls(self) -> None:
        body = "Ｔｉｔｌｅ\nThe agency\u202e must report income changes under 42 CFR part 435.".encode("utf-8")
        document = extract_text("https://example.gov/rules", "text/plain", body, parser_mode="no-bs4")
        self.assertIn("Title", document.text)
        self.assertNotIn("\u202e", document.text)

    def test_document_quality_flags_scan_full_text(self) -> None:
        body = ("<html><body><p>" + ("ordinary text " * 400) + " enable javascript </p></body></html>").encode("utf-8")
        document = extract_text("https://example.gov/rules", "text/html", body)
        self.assertIn("low_quality_browser_warning", document.extractor_notes)
