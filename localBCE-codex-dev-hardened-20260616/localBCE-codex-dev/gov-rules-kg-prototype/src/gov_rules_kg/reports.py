...
def build_executable_candidates(rules: list[dict]) -> list[dict]:
    ...
    for rule in rules:
        ...
        if rule.get("human_review_required"):
            continue
        if float(rule.get("confidence_score", 0)) < 0.85:
            continue
        if not rule.get("normalized_citation") or not rule.get("exact_source_text"):
            continue
        candidates.append(
            {
                "rule_id": rule["rule_id"],
                "exact_source_text": rule.get("exact_source_text"),
                "candidate_only": True,
                "executable_status": "requires_signed_ratification",
                "validation_status": "not_exported_unsigned",
