...
class DiscoveryEngine:
    ...
    def run(self) -> dict:
        ...
        while queue:
            ...
            source_snapshot = source_snapshot_object(fetch, self.config)
            source_snapshot_commitment = source_snapshot_hash(source_snapshot)
            self.log_progress(
                {
            ...
            if fetch.status != "FETCHED":
                self.store.upsert_document(
                    ...
                    "",
                    {
                        ...
                        "source_snapshot_commitment": source_snapshot_commitment,
                    },
                )
                continue
            ...
            metadata = {
                ...
                "source_snapshot": source_snapshot,
                "source_snapshot_commitment": source_snapshot_commitment,
                "source_content_hash": fetch.content_hash,
...
def source_snapshot_hash(snapshot: dict) -> str:
    payload = json.dumps(snapshot, sort_keys=True, separators=(",", ":")).encode("utf-8")
    return hashlib.sha256(payload).hexdigest()


def source_snapshot_object(fetch, config: RunConfig) -> dict:
    canonical_host = (urlparse(fetch.canonical_url).hostname or "").lower()
    source_host = (urlparse(fetch.url).hostname or "").lower()
    final_url = fetch.final_url or fetch.canonical_url
    final_host = (urlparse(final_url).hostname or "").lower()
    matching_seeds = [
        source.name
        for source in config.seeds
        if canonicalize_url(source.url) in {fetch.canonical_url, fetch.url}
    ]
    expected_hash = None
    for source in config.seeds:
        if canonicalize_url(source.url) in {fetch.canonical_url, fetch.url} and source.expected_sha256:
    ...
    return {
        ...
        "allowed_host_verified": canonical_host in {host.lower() for host in config.allowed_hosts},
        "final_host_allowed": final_host in {host.lower() for host in config.allowed_hosts},
        "manifest_seed_names": sorted(matching_seeds),
        "status": fetch.status,
        "status_code": fetch.status_code,
        "content_type": fetch.content_type,
        "content_sha256": fetch.content_hash,
        "expected_sha256": expected_hash,
        "expected_sha256_matched": None if not expected_hash or not fetch.content_hash else expected_hash == fetch.content_hash.lower(),
        "body_size_bytes": len(fetch.body),
        "attempts": fetch.attempts,
        "detail": fetch.detail,
    }
...
def enrich_atomic_rule(
    ...
    jurisdiction_level: str,
    state_code: str | None,
    source_snapshot_commitment: str,
