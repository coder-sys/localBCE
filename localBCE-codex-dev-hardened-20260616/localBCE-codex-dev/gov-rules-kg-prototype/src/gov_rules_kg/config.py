...
def default_seeds() -> list[Source]:
    return [
        Source("example", "https://example.com", required=False, discovery_seed=False),
...
def source_from_mapping(row: dict) -> Source:
    return Source(
        ...
        expected_sha256=str(row["expected_sha256"]).strip().lower() if row.get("expected_sha256") else None,
    )
...
def make_config(
    ...
    derived_allowed_hosts = sorted(
        {
            host
            for source in seeds
            for host in [urlparse(source.url).hostname]
            if host
        }
    )
    ...
    config = RunConfig(
        ...
        allowed_hosts=sorted(set(allowed_hosts or derived_allowed_hosts)),
        per_host_failure_cap=per_host_failure_cap,
