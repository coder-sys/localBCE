...
def normalize_claude_rules(
    ...
    for item in raw_rules:
        ...
        statement_grounded = evidence_matches_source(statement, source_text) if source_text else True
        human_review_required = not evidence_grounded or not statement_grounded
        reasons: list[str] = []
        if not evidence_grounded:
            reasons.append("Claude evidence excerpt was not found in the provided source text.")
        if not statement_grounded:
            reasons.append("Claude statement text diverged from the provided source text.")
