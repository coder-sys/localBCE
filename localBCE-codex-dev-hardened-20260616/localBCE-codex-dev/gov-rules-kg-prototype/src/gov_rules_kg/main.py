...
def main() -> None:
    ...
    if args.command == "export-executable":
        ...
        unsigned = [
            ...
            if not isinstance(item, dict) or item.get("validation_status") != "human_ratified_signed"
        ]
        payload = {
            "executable_rule_candidates": str(path),
            "items": len(candidates),
            "min_confidence": args.min_confidence,
            "unsigned_or_unratified_items": len(unsigned),
            "verdict": "EXPORT_BLOCKED_SIGNED_RATIFICATION_REQUIRED" if unsigned else "EXPORT_READY_SIGNED_RATIFIED",
        }
        print(json.dumps(payload, indent=2, sort_keys=True))
        if unsigned:
            raise SystemExit(2)
