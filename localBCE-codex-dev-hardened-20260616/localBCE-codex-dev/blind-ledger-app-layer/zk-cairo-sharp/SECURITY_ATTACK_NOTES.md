...
Status: CLOSED IN APP-LAYER HASHING; STILL PENDING CRYPTO AUDIT AND LIVE
PROVER INTEGRATION.

The app-layer duplicate/nullifier hash now uses the Cairo/Stark prime field via
`starknet-crypto 0.8.1`, and raw Poseidon rows must start with a recognized
domain tag. This removes the prior cross-field modular-reduction collision risk
from the app/nullifier stack.

Current state:

- `app/batch/poseidon.py` and the indexed nullifier tree use the Stark field
  through `starknet-crypto`.
- `zk-cairo-sharp/src/lib.cairo` uses Cairo `felt252` values and Cairo native
  Poseidon over the STARK field.
- Existing batch/contract roots remain `bytes32` commitments/public inputs; the
  native STARK settlement contract pins the verifier artifact and rejects legacy
  proof wrappers rather than pretending to recompute Poseidon in Solidity.

Why this matters:

The production path must avoid lossy field conversion. A value outside the STARK
field cannot be represented in Cairo without explicit decomposition or a
separate audited bridge. Values that differ before conversion can collide if a
bridge simply reduces them modulo the STARK field.
...
- add a separately specified and audited non-lossy bridge.
