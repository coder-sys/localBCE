...
contract BatchClaimsRegistry {
    ...
    uint256 public constant DEFAULT_BATCH_PUBLIC_INPUT_LENGTH = 20;
    bytes32 public constant CANONICAL_NULLIFIER_GENESIS = 0x111df93687d686f128495834e87222c67d3ec42ae5d128db3ad65e8123b11f2c;
    bytes32 public constant CLAIM_SOURCE_ROOT_KEY = keccak256("CLAIM_SOURCE_ROOT");
    bytes32 public constant ORACLE_FACTS_ROOT_KEY = keccak256("ORACLE_FACTS_ROOT");
    bytes32 public constant ORACLE_SIGNER_ROOT_KEY = keccak256("ORACLE_SIGNER_ROOT");
    bytes32 public constant FEE_SCHEDULE_ROOT_KEY = keccak256("FEE_SCHEDULE_ROOT");
    bytes32 public constant ADDRESS_BOOK_ROOT_KEY = keccak256("ADDRESS_BOOK_ROOT");
    bytes32 public constant DATA_AVAILABILITY_ROOT_KEY = keccak256("DATA_AVAILABILITY_ROOT");
    bytes32 public constant DENIAL_ATTESTATION_ROOT_KEY = keccak256("DENIAL_ATTESTATION_ROOT");
    bytes32 public constant FORCED_INCLUSION_ROOT_KEY = keccak256("FORCED_INCLUSION_ROOT");
    ...
    mapping(bytes32 => mapping(bytes32 => bool)) public approvedRoots;
    ...
    event RootApproved(bytes32 indexed rootKind, bytes32 indexed root, bool approved);
    ...
    constructor(address initialAdmin, bytes32 defaultVerifierKeyId, IBatchVerifier defaultChecker) {
    ...
    function setRootApproved(bytes32 rootKind, bytes32 root, bool approved) external onlyAdmin {
        require(_isKnownRootKind(rootKind), "unknown root kind");
        require(root != bytes32(0), "root zero");
        approvedRoots[rootKind][root] = approved;
        emit RootApproved(rootKind, root, approved);
    }
    ...
    function submitBatch(
        ...
        require(approvedRoots[CLAIM_SOURCE_ROOT_KEY][submission.claimSourceRoot], "claim source root not approved");
        require(approvedRoots[ORACLE_FACTS_ROOT_KEY][submission.oracleFactsRoot], "oracle facts root not approved");
        require(approvedRoots[ORACLE_SIGNER_ROOT_KEY][submission.oracleSignerRoot], "oracle signer root not approved");
        require(approvedRoots[FEE_SCHEDULE_ROOT_KEY][submission.feeScheduleRoot], "fee schedule root not approved");
        require(approvedRoots[ADDRESS_BOOK_ROOT_KEY][submission.addressBookRoot], "address book root not approved");
        require(submission.dataAvailabilityRoot != bytes32(0), "data availability root zero");
        require(submission.denialAttestationRoot != bytes32(0), "denial attestation root zero");
        require(submission.forcedInclusionRoot != bytes32(0), "forced inclusion root zero");
        ...
        stored.claimSourceRoot = submission.claimSourceRoot;
        stored.oracleFactsRoot = submission.oracleFactsRoot;
        stored.oracleSignerRoot = submission.oracleSignerRoot;
        stored.feeScheduleRoot = submission.feeScheduleRoot;
        stored.addressBookRoot = submission.addressBookRoot;
        stored.dataAvailabilityRoot = submission.dataAvailabilityRoot;
        stored.denialAttestationRoot = submission.denialAttestationRoot;
        stored.forcedInclusionRoot = submission.forcedInclusionRoot;
        stored.valueConservationCommitment = submission.valueConservationCommitment;
    ...
    function _batchInputsMatch(BatchSubmission calldata submission, bytes32[] calldata batchInputs) internal view returns (bool) {
        ...
        return batchInputs.length == publicInputLengths[submission.verifierKeyId]
            ...
            && batchInputs[11] == submission.claimSourceRoot
            && batchInputs[12] == submission.oracleFactsRoot
            && batchInputs[13] == submission.oracleSignerRoot
            && batchInputs[14] == submission.feeScheduleRoot
            && batchInputs[15] == submission.addressBookRoot
            && batchInputs[16] == submission.dataAvailabilityRoot
            && batchInputs[17] == submission.denialAttestationRoot
            && batchInputs[18] == submission.forcedInclusionRoot
            && batchInputs[19] == submission.valueConservationCommitment;
    }

    function _isKnownRootKind(bytes32 rootKind) internal pure returns (bool) {
        return rootKind == CLAIM_SOURCE_ROOT_KEY
            || rootKind == ORACLE_FACTS_ROOT_KEY
            || rootKind == ORACLE_SIGNER_ROOT_KEY
            || rootKind == FEE_SCHEDULE_ROOT_KEY
            || rootKind == ADDRESS_BOOK_ROOT_KEY
            || rootKind == DATA_AVAILABILITY_ROOT_KEY
            || rootKind == DENIAL_ATTESTATION_ROOT_KEY
            || rootKind == FORCED_INCLUSION_ROOT_KEY;
    }
