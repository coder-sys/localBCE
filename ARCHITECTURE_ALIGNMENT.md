...
| Claim-source, oracle-facts, fee roots | `blind-ledger-app-layer/app/batch/` and hardened bundle | `[REFERENCE]` | Not active in one-claim Groth16 runtime. |
| Indexed nullifier root transition | `blind-ledger-app-layer/app/batch/` and hardened bundle | `[REFERENCE]` | Active ClaimsRegistry has duplicate claim checks, but not the hardened indexed nullifier tree. |
...
| Hardened claim witness serializer | `blind-ledger-app-layer/zk-stark/`, `zk-cairo-sharp/`, hardened bundle | `[REFERENCE]` | Active `stark-engine/` has proof intent and witness plan models only. |
| Cairo/STARK statement | `blind-ledger-app-layer/zk-cairo-sharp/` and hardened bundle | `[REFERENCE]` | No real STARK proof is generated by active localBCE. |
...
adapter contract. It does not prove the claim, does not import Winterfell, does
not run Cairo, does not replace Groth16, and does not submit on-chain.
