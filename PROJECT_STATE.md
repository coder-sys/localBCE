...
- stark-engine/ now has a feature-gated canonical `feeScheduleRoot` artifact
  and planning-only inputs for nullifier root transitions.
...
- A feature-gated oracleFactsRoot candidate now commits a source manifest,
  normalized verified facts, HTTPS source references, and attestation
  references into a canonical depth-10 Rp64_256 Merkle tree. The production
  AIR constrains its leaf, path, and root. The references are committed, not
  externally attested or governed.
- A feature-gated feeScheduleRoot candidate now commits a verified HTTPS USD
  fee schedule, exact effective-date matching, and service-line allowance
  checks into a canonical depth-10 Rp64_256 Merkle tree. The production AIR
  constrains its leaf, path, root, and claim-source service digest, date, and
  total-charge links. Fee schedule governance remains external and disabled.
...
- The production proof artifact and verifier handoff use version 5 schemas.
  Their 26 public inputs include AIR-constrained `publicInputRoot`,
  `claimSourceRoot`, `oracleFactsRoot`, and `feeScheduleRoot` values packed
  canonically into `bytes32`.
- Three governed state roots remain unavailable, so the STARK verifier
...
- `stark-engine/` is the STARK bridge and feature-gated production AIR crate.
  ...
  implemented. A versioned handoff now locks the proof and 22 AIR inputs,
  including constrained claim-source and oracle-facts roots, to the Solidity
  ABI candidate, but remains non-call-ready because four root implementations,
  governance, runtime selection, and on-chain verification are incomplete.
