...
- stark-engine/ now has planning-only source inputs for feeScheduleRoot and
  nullifier root transitions.
...
- The production proof artifact and verifier handoff use version 4 schemas.
  Their 22 public inputs include AIR-constrained `publicInputRoot`,
  `claimSourceRoot`, and `oracleFactsRoot` values packed canonically into
  `bytes32`.
- Four governed source/state roots remain unavailable, so the STARK verifier
