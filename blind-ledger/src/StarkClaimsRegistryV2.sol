...
/// @notice Governed, non-proxy STARK registry for the Sepolia controlled-attestation pilot.
contract StarkClaimsRegistryV2 is AccessControl, Pausable, ReentrancyGuard {
    ...
    event TreasuryTransferAccepted(address indexed previousTreasury, address indexed newTreasury);
    event DenialFeesRealized(address indexed treasury, uint256 amount);
    ...
    constructor(
        ...
        emit TreasuryConfigured(initialTreasury);
    }
    ...
    function acceptTreasury() external {
        ...
        emit TreasuryTransferAccepted(previousTreasury, msg.sender);
    }

    function realizeDenialFees(uint256 amount) external onlyRole(TREASURY_ROLE) {
        require(amount <= denialFeesAccrued, "Exceeds accrued");
        denialFeesAccrued -= amount;
        baseFeesCollected += amount;
        emit DenialFeesRealized(msg.sender, amount);
    }
